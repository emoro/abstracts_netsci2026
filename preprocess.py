...
# ── Parse keyword-based sessions from the original proposal ──────────────────
with open(f"{BASE}/netsci2026_session_proposal.txt") as f:
    text = f.read()
...
keyword_map = {}
for label, block in sessions_raw:
    for pid in re.findall(r'\[(\d+)\]', block):
        keyword_map[pid] = label.strip()
...
# ── Build nodes with both session assignments ─────────────────────────────────
nodes = []
for t in talks:
